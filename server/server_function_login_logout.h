...
void handle_login_request(char *data, int connfd)
{
    ...
    if (sscanf(data, "username: %49[^;]; password: %49s", username, password) != 2)
    {
        ...
        snprintf(buffer, MAXLINE, "Invalid data format: %s", data);
        send_message("LOGIN_RES", buffer, connfd);
        return;
    }
...
//    Kiểm tra trạng thái đăng nhập của tài khoản 
    ...
    if (status == 1)
    {
        ...
        memset(buffer, 0, sizeof(buffer));
        snprintf(buffer, MAXLINE, "User %s is already logged in", username);
        send_message("LOGIN_RES", buffer, connfd);
        return;
    }
    // for (int i = 0; i < MAX_CLIENTS; i++)
    // {
    //     if (clients_status[i].is_logged_in && strcmp(clients_status[i].username, username) == 0)
    //     {
